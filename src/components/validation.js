...
class Validation extends Component {
  ...
  componentDidMount() {
    ...
    console.log(uid);
    Axios.post(`http://${ipAddress}:3001/validation/${uid}`).then(
      (response) => {
        if (response.data) {
    ...
    const data = orderInfo.map((item, index) => {
      return (
        <tr key={index}>
          ...
          <td className="align-middle text-center" style={{ width: "18%" }}>
            {item.Model_Name}
          </td>
          <td className="align-middle text-center" style={{ width: "18%" }}>
          ...
          <td style={{ width: "8%" }} className="align-middle text-center">
          ...
          <td style={{ width: "23%" }} className="align-middle text-center">
            ...
            <button style={{ width: 80 }} className="btn btn-danger btn-sm m-1">
              Invalid
            </button>
          </td>
        </tr>
        ...
        <div className="container">
          ...
          <div>
            <table className=" table table-striped table-bordered ">
              ...
              <tbody className="justify-content-center">{data}</tbody>
            </table>
          </div>
        </div>
      </div>
